...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Optionally, start in dark mode if preferred
    // body.classList.add('dark-theme');
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (key === 'Escape') btnText = 'C';
        if (key === '%') btnText = '%';
        if (key === '(' || key === ')') btnText = '()';

        if (btnText) {
            const btn = Array.from(buttons).find(b => b.innerText === btnText);
            if (btn) {
                btn.click();
                animateButton(btn);
            }
            ...
            backspaceBtn.click();
            // Add animation to backspace button if desired, though it's an SVG inside a button
            backspaceBtn.classList.add('pressed'); // We need to add .pressed style for backspace-btn if it's not .btn
